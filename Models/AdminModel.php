...
class AdminModel extends Mysql{
    ...
    {
        $sql = "SELECT u.id, u.nombre, l.id, l.titulo, p.id, p.id_usuario, p.id_libro, p.fecha_prestamo, p.fecha_devolucion, p.cantidad, p.observacion, p.estado FROM usuarios u INNER JOIN libro l INNER JOIN prestamo p ON p.id_usuario = u.id WHERE p.id_libro = l.id AND p.estado = 1";
        $res = $this->select_all($sql);
        return $res;
    }
}
